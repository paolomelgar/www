...
io.on('connection',function(socket){ 
	...
	socket.on('con',function(data){
		socket.emit('usuario',Array.from(new Set(usuarios.map(x=> x[0]))));
	})
	...
	socket.on('disconnect',function(data){ 
		...
		var index;
		for(var i = 0; i < usuarios.length; i++) {
		   if(usuarios[i][1] == socket.usuario) {
		     index=i;
		   }
		}
	...
	socket.on('notificacion',function(data){ 
		...
		var query = connection.query('SELECT * FROM total_pedido WHERE entregado="NO"', function(error, result){
	        if(error){
	           ...
	           count = result.length;
			   io.sockets.in("caja").emit('notificacion',count);
		    }
		});
