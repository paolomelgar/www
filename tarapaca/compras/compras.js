        ...
        $.ajax({
          ...
          success: function(data){
            ...
            for (var i=0;i<data[0].length;i++) {
              $('#row').append(
                "<tr class='fila'>\n" +
                "  <td width='2%' style='padding:0px;' title='s'><img src='https://raw.githubusercontent.com/paolomelgar/ferreboom/master/huancayo/producto/a"+data[0][i][4]+".jpg?timestamp=23124' width='100%' height='100%' class='img'></td>\n"+
...
$('#form select[name="forma-pago"]').val('');
  $('#documento').change(function(){
    ...
    else if($('#documento').val()=='FACTURA PAOLA' || $('#documento').val()=='FACTURA DORIS'){
      $('#forma-pago option[value=CONTADO]').show();
      $('#forma-pago option[value=CREDITO]').show();
      $('#serie').show();
      $('#numero').show();
