<?php
  ...
  if(isset($_POST) && !empty($_POST)){
    ...
    elseif($_POST['com']=='FACTURA PAOLA'){
    ...
    elseif($_POST['com']=='FACTURA DORIS'){
      $del = mysqli_query($con,"SELECT id,cantidad,entregado FROM facturadoris WHERE seriefactura='".$_POST['serie']."'"); 
      while($row = mysqli_fetch_assoc($del)){
        $ins=mysqli_query($con,"UPDATE producto SET stock_con1=(stock_con1+".$row['cantidad'].") WHERE id='".$row['id']."'");
      }
      $sql12=mysqli_query($con,"UPDATE facturadoris SET entregado='ANULADO' WHERE seriefactura='".$_POST['serie']."'");
    }
