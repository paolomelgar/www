...
if(isset($_POST) && !empty($_POST)){
	...
	case 'FACTURA PAOLA':
		if ($_POST['serieventa']==0) {
            $res = mysqli_query($con,"SELECT MAX(serieventas)+1 FROM total_ventas WHERE documento='FACTURA PAOLA'"); 
            $row = mysqli_fetch_row($res);
            $num="000000".$row[0];
            $num=substr($num,-7);
            for ($i=0; $i<sizeof($_POST['producto']) ; $i++) {
            ...
            $del2 = mysqli_query($con,"DELETE FROM total_ventas WHERE serieventas='".$_POST['serieventa']."' AND documento='FACTURA PAOLA'");
            $num=$_POST['serieventa'];
            for ($i=0; $i<sizeof($_POST['producto']) ; $i++) {
                $sql= mysqli_query($con,"INSERT INTO facturapaola (seriefactura,documento,id,compra,producto,cantidad,unitario,importe,especial,cliente,fecha,hora,vendedor,entregado) 
                    VALUES ('".$num."',
    ...
    case 'FACTURA DORIS':
        if ($_POST['serieventa']==0) {
            $res = mysqli_query($con,"SELECT MAX(serieventas)+1 FROM total_ventas WHERE documento='FACTURA DORIS'"); 
            $row = mysqli_fetch_row($res);
            $num="000000".$row[0];
            $num=substr($num,-7);
            for ($i=0; $i<sizeof($_POST['producto']) ; $i++) {
                $sql= mysqli_query($con,"INSERT INTO facturadoris (seriefactura,documento,id,compra,producto,cantidad,unitario,importe,especial,cliente,fecha,hora,vendedor,entregado) 
                    VALUES ('".$num."',
                            'FACTURA DORIS',
                            '".$_POST['id'][$i]."',
                            '".$_POST['compra'][$i]."',
            ...
            $del = mysqli_query($con,"SELECT id,cantidad,entregado FROM facturadoris WHERE seriefactura='".$_POST['serieventa']."'"); 
            while($row = mysqli_fetch_assoc($del)){
                if($row['entregado']=='NO' ){
                    $ins=mysqli_query($con,"UPDATE producto SET stock_con1=(stock_con1+".$row['cantidad'].") WHERE id='".$row['id']."'");
                }
            }
            $del1 = mysqli_query($con,"DELETE FROM facturadoris WHERE seriefactura='".$_POST['serieventa']."'");
            $del2 = mysqli_query($con,"DELETE FROM total_ventas WHERE serieventas='".$_POST['serieventa']."' AND documento='FACTURA DORIS'");
            $num=$_POST['serieventa'];
            for ($i=0; $i<sizeof($_POST['producto']) ; $i++) {
                $sql= mysqli_query($con,"INSERT INTO facturadoris (seriefactura,documento,id,compra,producto,cantidad,unitario,importe,especial,cliente,fecha,hora,vendedor,entregado) 
                    VALUES ('".$num."',
