...
while($s=mysqli_fetch_assoc($sls)){
	...
	$pdf->Image('C:/Users/Desktop/Documents/GitHub/ferreboom/huanuco/producto/a'.$s['id'].'.jpg', $pdf->GetX()+10, $pdf->GetY()-55,75,75);
	$n++;
	$pdf->Ln(30);
...
while($r=mysqli_fetch_assoc($query)){
	...
	$pdf->AddPage();
	$pdf->Cell(210,297,$pdf->Image('C:/Users/Desktop/Documents/GitHub/ferreboom/huanuco/familia/a'.$r['id'].'.jpg', 0, 0,210,297),'','','C');
	//$pdf->AddPage();
	$pdf->Cell(0,0," ",'','','C');
	$pdf->Ln(0);
	$i=0;
	...
	while($row=mysqli_fetch_assoc($sql)){
		...
		$r=$pdf->GetX();
		$pdf->SetX($r+8);
		$pdf->Cell(54,54,$pdf->Image('C:/Users/Desktop/Documents/GitHub/ferreboom/huanuco/producto/a'.$row['id'].'.jpg', $pdf->GetX()+2, $pdf->GetY()+2,54,54),'','','C');
		$i++;
		$m++;
		if($i>2){
			$pdf->Ln();
			for($j=0;$j<3;$j++){
				...
				if(mysqli_num_rows($sq)>0){
					$pdf->Cell(25,5,$pdf->Image('C:/Users/Desktop/Documents/GitHub/ferreboom/huanuco/marca/a'.$row1[0].'.jpg', $pdf->GetX()+35, $pdf->GetY(),25,5),'','','C');
				}else{
				...
				if($stock[$j]<=0){
					$pdf->Cell(49,35,$pdf->Image('C:/wamp/www/ventasfuera/agotado.png', $pdf->GetX()-54, $pdf->GetY()+10,49,35),'','','C');
				}else{
					if(strtotime(date("Y-m-d"))-strtotime($nuevo[$j])<30*24*60*60){
						$pdf->Cell(54,54,$pdf->Image('C:/wamp/www/ventasfuera/nuevo.png', $pdf->GetX()-62, $pdf->GetY(),54,54),'','','C');
					}
				}
				$pdf->SetXY($x+62, $y);
			}
