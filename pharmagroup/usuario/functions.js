$(function(){
	$.ajax({
        ...
        success: function(data){                                                    
              ...
              $('#cantidad').text($('#resultado').find('td:eq(6)').text());
              $('#cant').text(Math.ceil($('#resultado').find('td:eq(6)').text()/$('#pagina').val()));
                if (Math.ceil($('#resultado').find('td:eq(6)').text()/$('#pagina').val())==1) {
					$('#siguiente').hide();
					$('#ultimo').hide();
		        }else{
		        	$('#siguiente').show();
		...
		$(".selected").each(function(){
			...
			$("#agregardatos").dialog({
				...
				open:function(){
					...
					$('#formagregar input[id="nombre"]').val(dato1);
					$('#formagregar input[id="usuario"]').val(dato2);
					$('#formagregar input[id="password"]').val(dato3);
					$('#formagregar select[id="cargo"]').val(dato4);
					$('#formagregar select[id="activo1"]').val(dato5);
					$("#nombre").autocomplete({
					    source:"../maps/cliente.php",
					    minLength:1
					});
        ...
        $.ajax({
            ...
            success: function(data){   
				...
				if (Math.ceil($('#resultado').find('td:eq(6)').text()/$('#pagina').val())==1) {
					$('#siguiente').hide();
					$('#ultimo').hide();
		        }else if(Math.ceil($('#resultado').find('td:eq(6)').text()/$('#pagina').val())<1){
	...
	$('#siguiente').click(function(){
		...
		if($('#numero').val()>=Math.ceil($('#resultado').find('td:eq(6)').text()/$('#pagina').val())){
			$('#siguiente').hide();
			$('#ultimo').hide();
		}
		$.ajax({
	...
	$('#ultimo').click(function(){
		$('#numero').val(Math.ceil($('#resultado').find('td:eq(6)').text()/$('#pagina').val()));
		$.ajax({
            type: "POST",
            url: "mn.php",
            data: {
