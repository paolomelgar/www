...
require_once('../connection.php');
  if(isset($_POST) && !empty($_POST)){
    ...
    if($_SESSION['cargo']=='CAJERO FIERROS'){
        $producto.="almacen='FIERROS' AND ";
    }else{
        $producto.="almacen='TIENDA' AND ";
    }
    foreach($search AS $s){
    ...
    ?>
      <tr class="tr" bgcolor='white' <?php if($row['stock_real']<=0 && $_POST['doc']!='FACTURA'){echo "style='font-weight:bold;background-color:#FF9D9D;'";}else{echo "style='font-weight:bold;'";} ?>>
        <td style='padding:0px;' title='s'><?php echo '<img src="../fotos/a'.$row['id'].'.jpg?timestamp=23124" width="100%" height="100%">'; ?></td>
        ...
        <?php } ?>
        <td style='text-align:right;background-color:#f63'><?php echo $row['p_mayor']; ?></td>
