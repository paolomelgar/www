...
switch ($_POST['t']) {
	...
	case 'hora':
		for($j=0;$j<12;$j++){
			$quer = mysqli_query($con,"SELECT SUM(total) FROM total_ventas WHERE entregado='SI' AND MONTH(fecha)='".($_POST['m']+1)."' AND YEAR(fecha)='".$_POST['y']."' AND HOUR(hora) between '".($j+8)."' and '".($j+9)."'");
			$rr=mysqli_fetch_row($quer);
			$a[$j]=$rr[0];
			switch ($j) {
				case '0':
