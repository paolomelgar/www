...
<head>
	...
	<script type="text/javascript">
		$(function(){
			...
			$('#sel').change(function(){
				if($(this).val()=='totalproductos'){
					$.ajax({
				      type: "POST",
				      url: "query.php",
				      data: "query=SELECT * FROM producto WHERE activo='SI'",
				      cache: false,
				      success: function(data){
				        $('#result').append(data);
				        $( "th" ).resizable({ grid: [1, 10000] });
			...
			$('#sel').change(function(){
				if($(this).val()=='sinproveedor'){
					$.ajax({
				      type: "POST",
				      url: "query.php",
				      data: "query=SELECT * FROM producto WHERE proveedor='' AND activo='SI'",
				      cache: false,
				      success: function(data){
				        $('#result').append(data);
				        $( "th" ).resizable({ grid: [1, 10000] });
			...
			$('#sel').change(function(){
				if($(this).val()=='sinubicalmacen'){
					$.ajax({
				      type: "POST",
				      url: "query.php",
				      data: "query=SELECT * FROM producto WHERE ubicacion='' AND activo='SI'",
				      cache: false,
				      success: function(data){
				        $('#result').append(data);
				        $( "th" ).resizable({ grid: [1, 10000] });
			...
			$('#sel').change(function(){
				if($(this).val()=='sinubicmuestra'){
					$.ajax({
				      type: "POST",
				      url: "query.php",
				      data: "query=SELECT * FROM producto WHERE ubicacion2='' AND activo='SI'",
				      cache: false,
				      success: function(data){
				        $('#result').append(data);
				        $( "th" ).resizable({ grid: [1, 10000] });
...
<select id='sel'>
	...
	<option value='sinproveedor'>PRODUCTOS SIN PROVEEDOR</option>
	<option value='sinubicalmacen'>PRODUCTOS SIN UBICACION ALMACEN</option>
	<option value='sinubicmuestra'>PRODUCTOS SIN UBICACION MUESTRA</option>
	<option value='totalproductos'>LISTA TOTAL PRODUCTOS</option>
</select>
