...
while($s=mysqli_fetch_assoc($sls)){
	...
	$pdf->Image('D:/Mis documentos/Dropbox/Public/huanuco/producto/a'.$s['id'].'.jpg', $pdf->GetX()+10, $pdf->GetY()-55,75,75);
	$n++;
	$pdf->Ln(30);
...
while($r=mysqli_fetch_assoc($query)){
	...
	$pdf->Cell(190,200,$pdf->Image('D:/Mis documentos/Dropbox/Public/huanuco/familia/a'.$r['id'].'.jpg', $pdf->GetX()+45, $pdf->GetY()+20,100,100),'',1,'C');
	$pdf->AddPage();
	$i=0;
	$m=0;
	...
	while($row=mysqli_fetch_assoc($sql)){
		...
		$r=$pdf->GetX();
		$pdf->SetX($r+7);
		$pdf->Cell(85,75,$pdf->Image('D:/Mis documentos/Dropbox/Public/huanuco/producto/a'.$row['id'].'.jpg', $pdf->GetX()+1, $pdf->GetY(),85,75),'','','C');
		$i++;
		$m++;
		if($i>1){
			$pdf->Ln();
