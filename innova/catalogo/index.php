...
class PDF extends FPDF
{
   function Header()
    {
      	$this->Image('C:/wamp/www/catalogo/logo_innova.png',10,2,40);
   ...
   function Footer()
	{
		...
		$this->SetTextColor(0,80,180);
		$this->Cell(0,10,utf8_decode('PROLONGACION HUANUCO 258 - HUANCAYO'),0,0,'C');
...
while($s=mysqli_fetch_assoc($sls)){
	...
	$pdf->Image('../fotos/a'.$s['id'].'.jpg', $pdf->GetX()+10, $pdf->GetY()-55,75,75);
	$n++;
	$pdf->Ln(30);
...
$prod=array();
$stock=array();
$query=mysqli_query($con,"SELECT * FROM familia");
while($r=mysqli_fetch_assoc($query)){
	...
	while($row=mysqli_fetch_assoc($sql)){
		...
		$ar[$i]=utf8_decode(substr($row['producto'],0,25));
		$marca[$i]=$row['marca'];
		$prod[$i]="S/ ".$row['p_promotor'];
		$stock[$i]=$row['stock_real'];
		$r=$pdf->GetX();
		$pdf->SetX($r+8);
		$pdf->Cell(54,54,$pdf->Image('../fotos/a'.$row['id'].'.jpg', $pdf->GetX()+2, $pdf->GetY()+2,54,54),'','','C');
		$i++;
		$m++;
		if($i>2){
			$pdf->Ln();
			for($j=0;$j<3;$j++){
				...
				if($stock[$j]<=0){
					$pdf->Cell(49,35,$pdf->Image('C:/wamp/www/ventasfuera/agotado.png', $pdf->GetX()-30, $pdf->GetY()+10,49,35),'','','C');
				}
				$pdf->SetXY($x+62, $y);
			}
			$pdf->Ln(10);
