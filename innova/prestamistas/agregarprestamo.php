<?php 
  ...
  if(isset($_POST) && !empty($_POST)){
    ...
    for ($i=0; $i<sizeof($_POST['monto']) ; $i++) {
      $sql= mysqli_query($con,"INSERT INTO prestamistas (banco,dni,monto,fecha,pendiente,interes,porcentaje) VALUES ('".$_POST['banco']."','".$_POST['documento']."','".$_POST['monto'][$i]."','".date('Y-m-d', strtotime(str_replace('/', '-', $_POST['fecha'][$i])))."','SI',$cuotainteres,$porcentaje)");
    }
    $hoy=date("Y-m-d");
    ...
    $insert1=mysqli_query($con,"INSERT INTO ingresos (ingreso,tipo,monto,detalle,fecha,origen,sesion,usuario,mediopago) 
            VALUES ('INGRESO','PRESTAMO','".$_POST['total']."','".$_POST['banco']." - ".$_POST['documento']."',NOW(),'INNOVA','".$_SESSION['cargo']."','".$_SESSION['nombre']."','EFECTIVO')");
  }