<?php
	...
	function MontoMonetarioEnLetras($monto){
	    ...
	    $texto_con = " CON $monto_decimal/100 NUEVOS SOLES";
	    return NumerosALetras($monto).$texto_con;
	}
	require('../fpdf/fpdf.php');
	$pdf = new FPDF();
	$res=mysqli_query($con,"SELECT * FROM letraclientes WHERE value=(SELECT MAX(value) FROM letraclientes)");
	while($row=mysqli_fetch_assoc($res)){
		$pdf->AddPage();
		$pdf->SetFont('Arial','B',10);
		$pdf->SetY(36);
		$pdf->SetX(55);
		$pdf->Cell(15,10,"L/".$row['serie']);
		$pdf->SetX(77);
		$pdf->Cell(25,10,'F/'.$row['factura']);
		$pdf->SetX(105);
		$pdf->Cell(18,10,$row['fechagiro']);
		$pdf->SetX(130);
		$pdf->Cell(15,10,'Huancayo');
		$pdf->SetX(155);
		$pdf->Cell(20,10,$row['fechapago']);
		$pdf->SetX(183);
		$pdf->Cell(20,10,'S/. '.$row['total']);
		$pdf->SetY(48);
		$pdf->SetX(55);
		$pdf->Cell(150,10,MontoMonetarioEnLetras($row['total']));
		$pdf->SetFont('Arial','B',8);
		$pdf->SetY(59);
		$pdf->SetX(65);
		$pdf->Cell(60,10,$row['cliente']);
		$pdf->SetY(69);
		$pdf->SetX(65);
		$pdf->MultiCell(60,3,$row['direccion']);
		$pdf->SetY(73);
		$pdf->SetX(65);
		$pdf->Cell(60,10,$row['ruc']);
	}
	$pdf->Output();