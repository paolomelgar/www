        ...
        success: function(data){
          ...
          $('#tb1 tr').click(function(){
            ...
            if ($('#row'+result+' tr').hasClass('mayorstock')) {
              ...
              $('#id'+result).val(id);
              $('#ubic'+result).val(ubic);
              $('#caja'+result).val(caja);
      ...
      if($('#id').val()>0){
        ...
        if($('#cantidad').hasClass('mayorstock')){
          ...
          next += "<td width='10%' style='text-align:right'>" + parseFloat($("#importe").val()).toFixed(2) + "</td>\n" +
                  ...
                  "<td style='display:none'>" + $("#ubic").val() +"</td>\n" +
                  "<td style='display:none'>" + $("#caja").val() +"</td>\n" +
                  "</tr>";
        $('#cantidad').removeClass('mayorstock');
        $('#precio_u').removeClass('mayorstock');
        $('#row').append(next);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  $('#dialogver').on('click','.visualizar',function(){
    ...
    if($(this).parent().parent().find('td:eq(6)').text()=='SI'){
      $(".ui-dialog-buttonpane button:contains('EDITAR')").button("enable");
    }else if($(this).parent().parent().find('td:eq(6)').text()=='NO'){
      $(".ui-dialog-buttonpane button:contains('EDITAR')").button("enable");
    }
  });
  ...
  $("#guardarform").one('click',function(){
    if($('#id').val()>0 && $('#busqueda').val()!=""){
      ...
      $.ajax({
        ...
        success: function(data){
          /*var contenid;
          var x1 = screen.width/2 - 300/2;
          var y1 = screen.height/2 - 400/2;
          var w=window.open('','',"width=300,height=400,left="+x1+",top="+y1);
          var $tbody = $('#row');
          $tbody.find('tr').sort(function (a, b) {
            var tda = $(a).find('td:eq(9)').text();
            var tdb = $(b).find('td:eq(9)').text();
            return tda > tdb ? 1
                   : tda < tdb ? -1
                   : 0;
          }).appendTo($tbody);
          $('#print').append("<table width='100%' align='center' style='margin-top:-5px;font:0.5em Verdana;' align='center'><tr><td align='center'>Vendedor:"+$('#vendedor').val()+"</td></tr></table>\n"+
              "<table width='100%' align='center' style='margin-top:-5px;font:0.6em Verdana;'><tr><td align='center'>FECHA: "+$('#fecha').val()+"</td></tr></table>\n"+
              "<table width='100%' align='center' style='margin-top:-5px;font:0.6em Verdana;'><tr><td>CLIENTE: <span style='color:blue'>"+$('#razon_social').val()+"</span></td></tr></table>\n"+
              "<table width='100%' align='center' style='margin-top:-3px;margin-bottom:3px;font:0.6em Verdana;border:1px solid black'><tr><td align='center'>PEDIDO</td></tr></table>\n"
            );
            $('#row tr').each(function(){
              $('#print').append("<table width='100%' align='center' style='margin-top:-6px;font:0.6em Verdana;'><tr><td width='5%' align='center' rowspan='2' valign='top'><input style='width:10px;height:10px;border:1px solid black;margin-top:2px'></td><td width='95%' colspan='3'>"+$(this).find('td:eq(1)').text()+"</td></tr><tr><td width='30%' align='left'>Cant: <b>"+$(this).find('td:eq(2)').text()+"</b></td><td width='40%' align='left'>ubic: "+$(this).find('td:eq(9)').text()+"</td><td width='30%' align='left'>caja: "+$(this).find('td:eq(10)').text()+"</td></tr></table>");
            });
            if(parseFloat($('#devolucion').val())!=0){
              $('#print').append("<table width='100%' align='center' style='margin-top:-5px;margin-bottom:5px;font:0.7em Verdana;'><tr><td colspan='4' width='70%' align='right'>SUBTOTAL</td><td width='30%' align='right'>"+$('#subtotal').val()+"</td></tr></table><hr style='margin-top:-6px'>");
              $('#row1 tr').each(function(){
                $('#print').append("<table width='100%' align='center' style='margin-top:-6px;font:0.6em Verdana;'><tr><td width='5%' align='center' rowspan='2' valign='top'><input style='width:10px;height:10px;border:1px solid black;margin-top:2px'></td><td width='95%' colspan='3'>"+$(this).find('td:eq(1)').text()+"</td></tr><tr><td width='30%' align='left'>Cant: <b>"+$(this).find('td:eq(2)').text()+"</b></td><td width='40%' align='left'>ubic: "+$(this).find('td:eq(9)').text()+"</td><td width='30%' align='left'>caja: "+$(this).find('td:eq(10)').text()+"</td></tr></table>");
              });
              $('#print').append("<table width='100%' align='center' style='margin-top:-5px;font:0.7em Verdana;'><tr><td colspan='4' width='70%' align='right'>DEVOLUCION</td><td align='right' width='30%'>"+$('#devolucion').val()+"</td></tr></table>");
            }
            $('#print').append("<table width='100%' align='center' style='margin-top:-5px;font:0.7em Verdana;'><tr><td colspan='4' width='70%' align='right'>TOTAL</td><td align='right' width='30%'>"+$('#total').val()+"</td></tr></table>");
            
            if($('#comentario').val()!=''){
              $('#print').append("<table width='100%' align='center' style='margin-top:1px'><tr><td style='font:0.6em Verdana' width='5%'>OBS:</td><td style='font:0.7em Verdana;font-weight:bold'>"+$('#comentario').val()+"</td></tr></table></div>");
            }
            contenid = document.getElementById("print");
