    ...
    while ( $day_num <= $days_in_month ) {
        ...
        if(mysqli_num_rows($sql)>0 || mysqli_num_rows($sql1)>0 || mysqli_num_rows($sql2)>0){
            while($row = mysqli_fetch_assoc($sql2)){
                $montos.="<div style='color:red;font-size:12px;margin-bottom:-12px;cursor:pointer;' title='".$row['banco']."<br/>S/. ".$row['monto']."<br/>DOC: ".$row['dni']."' class='prestamo' id='".$row['id']."'>".substr($row['banco'],0,10)."..".": S/. ".$row['monto']."</div><br>";
            }
