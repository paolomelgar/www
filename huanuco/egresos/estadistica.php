...
switch ($_POST['t']) {
	...
	case 'flete':
		for($j=0;$j<12;$j++){
			$quer = mysqli_query($con,"SELECT SUM(monto) FROM ingresos WHERE tipo='FLETE' AND MONTH(fecha)='".($j+1)."' AND YEAR(fecha)='".$_POST['y']."' AND ingreso!='ANULADO'");
			$rr=mysqli_fetch_row($quer);
			$a[$j]=$rr[0];
			switch ($j) {
				case '0':
	...
	case 'delivery':
		for($j=0;$j<12;$j++){
			$quer = mysqli_query($con,"SELECT SUM(monto) FROM ingresos WHERE tipo='DELIVERY' AND MONTH(fecha)='".($j+1)."' AND YEAR(fecha)='".$_POST['y']."' AND ingreso!='ANULADO'");
			$rr=mysqli_fetch_row($quer);
			$a[$j]=$rr[0];
			switch ($j) {
				case '0':
	...
	case 'impuestos':
	for($j=0;$j<12;$j++){
			$quer = mysqli_query($con,"SELECT SUM(monto) FROM ingresos WHERE tipo='IMPUESTOS' AND MONTH(fecha)='".($j+1)."' AND YEAR(fecha)='".$_POST['y']."' AND ingreso!='ANULADO'");
			$rr=mysqli_fetch_row($quer);
			$a[$j]=$rr[0];
			switch ($j) {
				case '0':
