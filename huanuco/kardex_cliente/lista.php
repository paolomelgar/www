...
require_once('../connection.php');
    ...
    else if($_POST['change']=='CONTABLE PAOLA'){
      $sql=mysqli_query($con,"SELECT * FROM facturapaola WHERE $query UNION SELECT * FROM boleta WHERE $query ORDER BY fecha,hora");
    }
    else{
      $sql=mysqli_query($con,"SELECT * FROM facturadoris WHERE $query ORDER BY fecha,hora");
    }
    $n=1;
    ?>
